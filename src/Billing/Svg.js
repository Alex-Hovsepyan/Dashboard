
function Svg({value, color, width, height}) {
        if(value === 'masterCard')
            return (
            <svg fill='#fff' width="50px" height="50px" viewBox="0 4 24 26"><title></title>
            <path d="M11.343 18.031c.058.049.12.098.181.146-1.177.783-2.59 1.238-4.107 1.238C3.32 19.416 0 16.096 0 12c0-4.095 3.32-7.416 7.416-7.416 1.518 0 2.931.456 4.105 1.238-.06.051-.12.098-.165.15C9.6 7.489 8.595 9.688 8.595 12c0 2.311 1.001 4.51 2.748 6.031zm5.241-13.447c-1.52 0-2.931.456-4.105 1.238.06.051.12.098.165.15C14.4 7.489 15.405 9.688 15.405 12c0 2.31-1.001 4.507-2.748 6.031-.058.049-.12.098-.181.146 1.177.783 2.588 1.238 4.107 1.238C20.68 19.416 24 16.096 24 12c0-4.094-3.32-7.416-7.416-7.416zM12 6.174c-.096.075-.189.15-.28.231C10.156 7.764 9.169 9.765 9.169 12c0 2.236.987 4.236 2.551 5.595.09.08.185.158.28.232.096-.074.189-.152.28-.232 1.563-1.359 2.551-3.359 2.551-5.595 0-2.235-.987-4.236-2.551-5.595-.09-.08-.184-.156-.28-.231z"/>
            </svg> 
            )   

        if(value === 'elipsis') {
            return (
                <svg fill='#fff' width="65px" height="20px" viewBox="0 5 1 35"> 		  
                <path d="M11.214,20.956c0,3.091-2.509,5.589-5.607,5.589C2.51,26.544,0,24.046,0,20.956c0-3.082,2.511-5.585,5.607-5.585C8.705,15.371,11.214,17.874,11.214,20.956z"/> 		                    
                <path d="M26.564,20.956c0,3.091-2.509,5.589-5.606,5.589c-3.097,0-5.607-2.498-5.607-5.589c0-3.082,2.511-5.585,5.607-5.585C24.056,15.371,26.564,17.874,26.564,20.956z"/>
                <path d="M41.915,20.956c0,3.091-2.509,5.589-5.607,5.589c-3.097,0-5.606-2.498-5.606-5.589c0-3.082,2.511-5.585,5.606-5.585C39.406,15.371,41.915,17.874,41.915,20.956z"/>                         
                </svg>  
            )
        }

        if(value === 'wave') {
            return (
                <svg width="61" height="50" viewBox="0 0 61 20" fill="none">                             
                    <path d="M1 8.2C1.70238 11.8 4.5 18.5 11 18C17.5 17.5 18.2786 1 29.6571 1C41.0357 1 41.0357 20.0286 60 3.57143" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg> 
            )
        }

        if(value === 'building') {
            return (
                <svg fill='rgb(1, 181, 116)' width={30} height={40} viewBox="0 0 176 176">                         
            <path d="M155.706,176c-0.001,0-0.003,0-0.005,0l-65.411-0.125H20.294c-1.381,0-2.5-1.119-2.5-2.5V60.904 	                           
            c0-1.381,1.119-2.5,2.5-2.5h34.911V2.5c0-1.381,1.119-2.5,2.5-2.5h98c1.381,0,2.5,1.119,2.5,2.5v171c0,0.664-0.264,1.3-0.734,1.77 	                            
            C157.003,175.737,156.368,176,155.706,176z M92.794,170.88l60.411,0.115V5h-93v53.404h30.089c1.381,0,2.5,1.119,2.5,2.5V170.88z 	                             
            M22.794,170.875h65V63.404h-65V170.875z M80.076,154.634H60.297c-1.381,0-2.5-1.119-2.5-2.5v-11.575c0-1.381,1.119-2.5,2.5-2.5 	                            
            h19.779c1.381,0,2.5,1.119,2.5,2.5v11.575C82.576,153.515,81.457,154.634,80.076,154.634z M62.797,149.634h14.779v-6.575H62.797 	                            
            V149.634z M50.292,154.634H30.513c-1.381,0-2.5-1.119-2.5-2.5v-11.575c0-1.381,1.119-2.5,2.5-2.5h19.779c1.381,0,2.5,1.119,2.5,2.5 	                           
            v11.575C52.792,153.515,51.673,154.634,50.292,154.634z M33.013,149.634h14.779v-6.575H33.013V149.634z M138.714,153.35h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5v-10.392c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C141.214,152.231,140.095,153.35,138.714,153.35z M131.652,148.35h4.563v-5.392h-4.563V148.35z M111.135,153.35h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5v-10.392c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C113.635,152.231,112.516,153.35,111.135,153.35z M104.072,148.35h4.563v-5.392h-4.563V148.35z M80.076,132.017H60.297 	                            
            c-1.381,0-2.5-1.119-2.5-2.5v-11.575c0-1.381,1.119-2.5,2.5-2.5h19.779c1.381,0,2.5,1.119,2.5,2.5v11.575 	                            
            C82.576,130.897,81.457,132.017,80.076,132.017z M62.797,127.017h14.779v-6.575H62.797V127.017z M50.292,132.017H30.513 	                            
            c-1.381,0-2.5-1.119-2.5-2.5v-11.575c0-1.381,1.119-2.5,2.5-2.5h19.779c1.381,0,2.5,1.119,2.5,2.5v11.575 	                            
            C52.792,130.897,51.673,132.017,50.292,132.017z M33.013,127.017h14.779v-6.575H33.013V127.017z M138.714,128.439h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5v-10.391c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.391 	                            
            C141.214,127.32,140.095,128.439,138.714,128.439z M131.652,123.439h4.563v-5.391h-4.563V123.439z M111.135,128.439h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5v-10.391c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.391 	                            
            C113.635,127.32,112.516,128.439,111.135,128.439z M104.072,123.439h4.563v-5.391h-4.563V123.439z M80.076,109.399H60.297 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V95.324c0-1.381,1.119-2.5,2.5-2.5h19.779c1.381,0,2.5,1.119,2.5,2.5v11.575 	                            
            C82.576,108.28,81.457,109.399,80.076,109.399z M62.797,104.399h14.779v-6.575H62.797V104.399z M50.292,109.399H30.513 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V95.324c0-1.381,1.119-2.5,2.5-2.5h19.779c1.381,0,2.5,1.119,2.5,2.5v11.575 	                            
            C52.792,108.28,51.673,109.399,50.292,109.399z M33.013,104.399h14.779v-6.575H33.013V104.399z M138.714,103.528h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V90.637c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C141.214,102.409,140.095,103.528,138.714,103.528z M131.652,98.528h4.563v-5.392h-4.563V98.528z M111.135,103.528h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V90.637c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C113.635,102.409,112.516,103.528,111.135,103.528z M104.072,98.528h4.563v-5.392h-4.563V98.528z M80.076,86.781H60.297 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V72.706c0-1.381,1.119-2.5,2.5-2.5h19.779c1.381,0,2.5,1.119,2.5,2.5v11.576 	                            
            C82.576,85.662,81.457,86.781,80.076,86.781z M62.797,81.781h14.779v-6.576H62.797V81.781z M50.292,86.781H30.513 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V72.706c0-1.381,1.119-2.5,2.5-2.5h19.779c1.381,0,2.5,1.119,2.5,2.5v11.576 	                            
            C52.792,85.662,51.673,86.781,50.292,86.781z M33.013,81.781h14.779v-6.576H33.013V81.781z M138.714,78.617h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V65.726c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.391 	                            
            C141.214,77.498,140.095,78.617,138.714,78.617z M131.652,73.617h4.563v-5.391h-4.563V73.617z M111.135,78.617h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V65.726c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.391 	                            
            C113.635,77.498,112.516,78.617,111.135,78.617z M104.072,73.617h4.563v-5.391h-4.563V73.617z M138.714,53.707h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V40.815c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C141.214,52.587,140.095,53.707,138.714,53.707z M131.652,48.707h4.563v-5.392h-4.563V48.707z M111.135,53.707h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V40.815c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C113.635,52.587,112.516,53.707,111.135,53.707z M104.072,48.707h4.563v-5.392h-4.563V48.707z M83.555,53.707h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V40.815c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C86.055,52.587,84.936,53.707,83.555,53.707z M76.493,48.707h4.563v-5.392h-4.563V48.707z M138.714,28.795h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V15.904c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C141.214,27.676,140.095,28.795,138.714,28.795z M131.652,23.795h4.563v-5.392h-4.563V23.795z M111.135,28.795h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V15.904c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C113.635,27.676,112.516,28.795,111.135,28.795z M104.072,23.795h4.563v-5.392h-4.563V23.795z M83.555,28.795h-9.563 	                            
            c-1.381,0-2.5-1.119-2.5-2.5V15.904c0-1.381,1.119-2.5,2.5-2.5h9.563c1.381,0,2.5,1.119,2.5,2.5v10.392 	                            
            C86.055,27.676,84.936,28.795,83.555,28.795z M76.493,23.795h4.563v-5.392h-4.563V23.795z"/>                     
            </svg> 
            )
        }

        if(value === 'coloredMasterCard') {
            return (
                <svg width="24px" height="24px" viewBox="0 0 24 24">                             
                    <circle cx="7" cy="12" r="7" fill="#EA001B"/>                             
                    <circle cx="17" cy="12" r="7" fill="#FFA200" fill-opacity=".8"/>                         
                </svg> 
            )
        }

        if(value === 'pencil') {
            return (
                <svg fill={color} width={width} height={height} viewBox="-20 5 80 65"> 	                    
                    <path d="M5.243,44.844L42.378,7.708l9.899,9.899L15.141,54.742L5.243,44.844z"/> 	                    
                    <path d="M56.521,13.364l1.414-1.414c1.322-1.322,2.05-3.079,2.05-4.949s-0.728-3.627-2.05-4.949S54.855,0,52.985,0 		                        
                            s-3.627,0.729-4.95,2.051l-1.414,1.414L56.521,13.364z"/> 	                    
                    <path d="M4.099,46.527L0.051,58.669c-0.12,0.359-0.026,0.756,0.242,1.023c0.19,0.19,0.446,0.293,0.707,0.293 		                        
                            c0.106,0,0.212-0.017,0.316-0.052l12.141-4.047L4.099,46.527z"/> 	                    
                    <path d="M43.793,6.294l1.415-1.415l9.899,9.899l-1.415,1.415L43.793,6.294z"/>                     
                </svg>  
            )
        }

        if(value === 'visa') {
            return (
                <svg fill='#fff' width="30px" height="30px" viewBox="0 0 750 471">                                 
                    <path id="Shape"fill="#fff" d="M278.198,334.228l33.36-195.763h53.358l-33.384,195.763H278.198   L278.198,334.228z" />                                 
                    <path id="path13" fill="#fff" d="M524.307,142.687c-10.57-3.966-27.135-8.222-47.822-8.222   c-52.725,0-89.863,26.551-90.18,64.604c-0.297,28.129,26.514,43.821,46.754,53.185c20.77,9.597,27.752,15.716,27.652,24.283   c-0.133,13.123-16.586,19.116-31.924,19.116c-21.355,0-32.701-2.967-50.225-10.274l-6.877-3.112l-7.488,43.823   c12.463,5.466,35.508,10.199,59.438,10.445c56.09,0,92.502-26.248,92.916-66.884c0.199-22.27-14.016-39.216-44.801-53.188   c-18.65-9.056-30.072-15.099-29.951-24.269c0-8.137,9.668-16.838,30.559-16.838c17.447-0.271,30.088,3.534,39.936,7.5l4.781,2.259   L524.307,142.687" /><path id="Path" fill="#fff" d="M661.615,138.464h-41.23c-12.773,0-22.332,3.486-27.941,16.234   l-79.244,179.402h56.031c0,0,9.16-24.121,11.232-29.418c6.123,0,60.555,0.084,68.336,0.084c1.596,6.854,6.492,29.334,6.492,29.334   h49.512L661.615,138.464L661.615,138.464z M596.198,264.872c4.414-11.279,21.26-54.724,21.26-54.724   c-0.314,0.521,4.381-11.334,7.074-18.684l3.607,16.878c0,0,10.217,46.729,12.352,56.527h-44.293V264.872L596.198,264.872z" /><path d="M 45.878906 138.46484 L 45.197266 142.53711 C 66.290228 147.64311 85.129273 155.0333 101.62305 164.22656 L 148.96875 333.91406 L 205.42383 333.85156 L 289.42773 138.46484 L 232.90234 138.46484 L 180.66406 271.96094 L 175.09961 244.83008 C 174.83893 243.99185 174.55554 243.15215 174.26562 242.31055 L 156.10547 154.99219 C 152.87647 142.59619 143.50892 138.89684 131.91992 138.46484 L 45.878906 138.46484 z "id="path16" />                             
                </svg> 
            )
        }

        if(value === 'file') {
            return (
                <svg width="25px" height="25px" viewBox="0 0 146 96">
                    <rect width="85" height="100"/>
                    <path  id="Path_191" data-name="Path 191" d="M10,84V12a8,8,0,0,1,8-8H62.563A8.3,8.3,0,0,1,68.43,6.43L83.57,21.57A8.3,8.3,0,0,1,86,27.437V84a8,8,0,0,1-8,8H18A8,8,0,0,1,10,84Z" fill="none" stroke="#58595b" stroke-linecap="round" stroke-miterlimit="10" stroke-width="4"/>
                    <path id="Path_192" data-name="Path 192" d="M64,5V17a8,8,0,0,0,8,8H84" fill="none" stroke="#58595b" stroke-linecap="round" stroke-miterlimit="10" stroke-width="4"/>
                    <line id="Line_26" data-name="Line 26" x2="42" transform="translate(27 46)" fill="none" stroke="#58595b" stroke-linecap="round" stroke-miterlimit="10" stroke-width="4"/>
                    <line id="Line_27" data-name="Line 27" x2="42" transform="translate(27 69)" fill="none" stroke="#58595b" stroke-linecap="round" stroke-miterlimit="10" stroke-width="4"/>
                </svg>
            )
        }

        if(value === 'delete') {
           return (
           <svg fill="red" viewBox="0 -8 30 50" width="25px" height="25px">
                <path d="M 13 3 A 1.0001 1.0001 0 0 0 11.986328 4 L 6 4 A 1.0001 1.0001 0 1 0 6 6 L 24 6 A 1.0001 1.0001 0 1 0 24 4 L 18.013672 4 A 1.0001 1.0001 0 0 0 17 3 L 13 3 z M 6 8 L 6 24 C 6 25.105 6.895 26 8 26 L 22 26 C 23.105 26 24 25.105 24 24 L 24 8 L 6 8 z"/>
            </svg>
           )
        }

        if(value === 'calendar') {
            return (
                <svg fill="#fff" width="18px" height="35px" viewBox="0 3 35 31">
		            <path d="M31.43,4.945c0-1.194-0.968-2.162-2.162-2.162h-3.656V0.882C25.612,0.395,25.217,0,24.73,0h-1.905
			                c-0.485,0-0.883,0.395-0.883,0.882v1.902H9.802V0.882C9.802,0.395,9.407,0,8.92,0H7.142C6.655,0,6.26,0.395,6.26,0.882v1.902H2.35
			                c-1.194,0-2.162,0.968-2.162,2.162v7.704h1.139v16.749c0,1.229,0.995,2.223,2.223,2.223h24.394c1.229,0,2.223-0.994,2.223-2.223
			                v-16.75h1.267L31.43,4.945L31.43,4.945z M26.624,25.73c0,1.229-0.994,2.223-2.223,2.223H7.218c-1.228,0-2.223-0.994-2.223-2.223
			                V12.649h21.628V25.73L26.624,25.73z"/>
		            <rect x="8.031" y="15.051" width="3.541" height="3.669"/>
		            <rect x="14.102" y="15.051" width="3.541" height="3.669"/>
		            <rect x="20.172" y="15.051" width="3.542" height="3.669"/>
                </svg>
            )
        }
    }


export default Svg